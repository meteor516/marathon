...
public interface PackGroupDao extends JpaRepository<PackGroup, String> {
    ...
    @Query(value = "select u from com.sf.marathon.domain.PackGroup u where u.id=:id and u.finish = 0 limit 1")
    PackGroup findUnfinishPackageGroupById(@Param("id") String pid);
}
